...
void ReadLandUseGeneral(DistributedHbv * const DistHbv, int nRows, int nCols, int noData, double xllCorner, double yllCorner, double cellSize, ifstream &fileControl, ofstream &fout)
{
  ...
  //  previousValue=-9999.0;
  cout << " File with soil types: " << endl;
  /*  cin >> fileName;
    cout << endl;*/
  fileControl.ignore(100, ':');
  fileControl >> fileName;
...
{
  int i, k, n, maxIndex;
  double lakePer, glacPer, areaCorr[2], totalArea, tempArea, areaFraction[numberLandSurfaceClasses-1];
  ...
  for (i=0; i<nRows*nCols; i++) {
    if (DistHbv[i].GetLandIndex()!=noData) {
      ...
      // Read explicit soil types insted of assigning implicit soil types based on land use
	...
	if (glacPer >= 50.0) {
	  landScapeOut.width(10); landScapeOut << SOIL(6) << "  ";
	}
	else if (landSurfType[k] == LANDSURFACE(2) || landSurfType[k] == LANDSURFACE(5)) {
	  landScapeOut.width(10); landScapeOut << SOIL(0) << "  ";
	}
	else {
