	                ...
	                double VPDopen, double gh, double cl, double z0g)
{
	...
	double mTmin, mVPD, VPD, Rcorr, Gs_st, Gs_cu, z0c,cd,d, z0;
  ...
  //###### Albedo estimation based on snow water equalvalent
	...
	if (treeLai < 0.01) {
		...
		VPD = (vas - vabar) * 1000;
		if (VPD <= VPDopen) mVPD = 1;
		if (VPD > VPDopen && VPD < VPDclose) mVPD = (VPDclose - VPD) / (VPDclose - VPDopen);
		if (VPD >= VPDclose) mVPD = 0;

		Rcorr = 1 / (101300 / pz / 1000 * pow(((temp + 273.15) / 293.15), 1.75));

		Gs_st = cl*mTmin*mVPD*Rcorr;

		Gs_cu = Rcorr*0.00001;
        surResist = 1 / (((Gs_st + Gs_cu)*gh)*treeLai / (Gs_st + Gs_cu + gh));
        if(surResist > bulkResist) surResist = bulkResist;
	}
...
//	}    //surResist = 0 for water surface
	...
	ETP =  (delta*Rn + airDensity*0.001013*(vas - vabar) / aeroResist) / lv / (delta + gamma*(1 + surResist / aeroResist));
    ETP_snow =  (delta*Rn_snow + airDensity*0.001013*(vas - vabar) / aeroResist) / lv / (delta + gamma*(1 + surResist / aeroResist));
  ...
  ETP_tot = ETP;
	...
	if(ETP < 0.00001) {
       ...
       /* cout << endl << " Tair =  " << temp ;
	cout << endl << " Tmax =  " << tempMax;
	cout << endl << " Tmin =  " << tempMin;
	cout << endl << " Rs =  " << radiationS;
	cout << endl << " Rns =  " << Rns;
    cout << endl << " Rn =  " << Rn;
    cout << endl << " R_nl =  " << R_nl;
    cout << endl << " R_so =  " << R_so;
    cout << endl << " sun_hour =  " << sun_hour;
    cout << endl << " lat_rad =  " << lat_rad;
	cout << endl << " vp =  " << vp ;
    cout << endl << " vas =  " << vas;
	cout << endl << " Wind =  " << wind1; 
	cout << endl << " DoY =  " << dayofyear;
	cout << endl << " LAT =  " << elemLatitute;
	cout << endl << " Elev =  " << elemElevation;
	cout << endl << " tree height =  " << treeHeight ; 
	cout << endl << " tree LAI =  " << treeLai;
	cout << endl << " tree LAI corr =  " << treeLaiCorr;
	cout << endl << " albedo =  " << treeAlbedo*(1.-snowCoverFraction)+ albedo_snow*snowCoverFraction;
	cout << endl << " snowCoverFraction =  " << snowCoverFraction ;
	cout << endl << " tree albedo =  " << treeAlbedo;
	cout << endl << " tree bulkResist =  " << bulkResist;
  cout << endl << " delta =  " << delta;
  cout << endl << " air density =  " << airDensity;
  cout << endl << " aeroResist =  " << aeroResist;
  cout << endl << " surResist =  " << surResist;
	cout << endl << " ETP =  " << ETP_tot << endl;  */ 
