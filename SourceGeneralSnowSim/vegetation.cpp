...
// CALC LAI and adjust for lower limit of LAI for forest alnm()
  ...
  potev = potentialEvap (temp, tempMax, tempMin, radiationS, vp, 
	  ...
	  landSurfacePar->GetDECIDUOUS_SHARE(), snowCoverFraction,landSurfacePar->GetWind_H(),landSurfacePar->GetTopen_min(), landSurfacePar->GetTclose_min(), landSurfacePar->GetVPDclose(),
	  landSurfacePar->GetVPDopen(), landSurfacePar->GetGH(), landSurfacePar->GetCL(), landSurfacePar->GetZ0G()) / 1000. ; // convert from mm/day to m/day
  ...
  if (precipitation > 0.0 || prevInterception > 0.0) {
    ...
    if (interceptionStore < 0.0-epsilon) {
