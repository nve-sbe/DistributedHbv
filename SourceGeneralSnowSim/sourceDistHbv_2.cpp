...
void WaterBalanceGrid(DistributedHbv * const DistHbv,  ParametersGeneral * ParGeneralStore, 
                      ...
                      int numLand, int timeStep, int nRows, int nCols, DateTime datetime, char * metPath, char * snowPath,
                      unsigned short int * precip10, unsigned short int * temp10K, unsigned short int * tmax10K, 
		      unsigned short int * tmin10K, unsigned short int * wind10, unsigned short int * solar10, unsigned short int * vp10, 
		      unsigned short int * swe10, unsigned char * lwc10, unsigned char * sca10, unsigned char * qtt10,
		      bool * inputDataFound, int * indexStore, int numberIndexStore, ofstream &fout);
...
int main(int argc, char *argv[])
{
  ...
  // Grid file format input data
   ...
   // Path to grid files with SeNorge snowsim data
    snowPath = getenv("SNOWDATA");
    if (!snowPath) {
      cout <<" Environment variable SNOWDATA not defined " << endl << endl;
      exit(1);
    }
    ...
    unsigned short int * swe10 = new unsigned short int[nRows*nCols];
    unsigned char * lwc10 = new unsigned char[nRows*nCols];
    unsigned char * sca10 = new unsigned char[nRows*nCols];
    unsigned char * qtt10 = new unsigned char[nRows*nCols];
    ...
    for (datetime=startModelTime; datetime<=endSimulationTime; datetime+=ParGeneralStore->GetSECONDS_TIMESTEP()) {
      ...
      WaterBalanceGrid(DistHbv, ParGeneralStore, InputElementStore, initialTimeSteps, numberTimeSteps, 
                       ...
                       datetime, metPath, snowPath, precip10, temp10K, tmax10K, tmin10K, wind10, solar10, vp10, 
		       swe10, lwc10, sca10, qtt10, &inputDataFound, indexStore, numberIndexStore, fout);
      // Traverse sub-catchments and landscape elements
      if (inputDataFound) {
        for (i=0; i<numWatcOut; i++) { 
          TraverseSubCatchment(Outlet[i], timeStep, fout);
...
void WaterBalanceGrid(DistributedHbv * DistHbv,  ParametersGeneral * ParGeneralStore, InputElement * InputElementStore,
		      ...
		      bool * inputDataFound, int * indexStore, int numberIndexStore, ofstream &fout)
{
  ifstream filePrec, fileTemp, fileTmax, fileTmin, fileSolar, fileWind, fileVP, fileSwe, fileLwc, fileSca, fileQtt;
  int i,j,k, jNew, dayofyear_PM2;
  int noSnow8u=254, noData8u=255, noData16u=65535;
  unsigned short int metMissing = 10000;
  double precipitation, preci_corr, temperature, Tmax, Tmin, wind, solarRadiation, vp;
  double sweSnowSim, lwcSnowSim, scaSnowSim,  qttSnowSim;
  char fileName[100];
  ...
  char VPFileName[100];
  /* SeNorge snowsim */
  char sweFileName[100];
  char lwcFileName[100];
  char scaFileName[100];
  char qttFileName[100];
  char hydYear[5];
  char metYear[5];
  ...
  sprintf(metYear,"%04d",datetime.getYear());
  strcat(precFileName,metYear);
  strcat(tempFileName,metYear);
  strcat(TmaxFileName, metYear);
  strcat(TminFileName, metYear);
  strcat(windFileName, metYear);
  strcat(solarFileName, metYear);
  strcat(VPFileName, metYear);

  /* SeNorge snowsim */
  strcpy(sweFileName,snowPath);
  strcpy(lwcFileName,snowPath);
  strcpy(scaFileName,snowPath);
  strcpy(qttFileName,snowPath);

  strcat(sweFileName,"/swe/");
  strcat(lwcFileName,"/lwc/");
  strcat(scaFileName,"/sca/");
  strcat(qttFileName,"/qtt/");
  ...
  sprintf(hydYear,"%04d",datetime.getYear());
  strcat(sweFileName,hydYear);
  strcat(lwcFileName,hydYear);
  strcat(scaFileName,hydYear);
  strcat(qttFileName,hydYear);
  ...
  /* SeNorge snowsim */
  sprintf(fileName, "/swe_%04d_%02d_%02d.bil", datetime.getYear(), datetime.getMonth(), datetime.getDay());
  strcat(sweFileName, fileName);

  sprintf(fileName, "/lwc_%04d_%02d_%02d.bil", datetime.getYear(), datetime.getMonth(), datetime.getDay());
  strcat(lwcFileName, fileName);

  sprintf(fileName, "/sca_%04d_%02d_%02d.bil", datetime.getYear(), datetime.getMonth(), datetime.getDay());
  strcat(scaFileName, fileName);

  sprintf(fileName, "/qtt_%04d_%02d_%02d.bil", datetime.getYear(), datetime.getMonth(), datetime.getDay());
  strcat(qttFileName, fileName);
  ...
  /* SeNorge snowsim */
  fileSwe.open(sweFileName,ios::in | ios::binary);
  if (!fileSwe.is_open()) {
    cout << endl << "Error opening file " << sweFileName << endl << endl;
    exit (1);
  }

  fileLwc.open(lwcFileName,ios::in | ios::binary);
  if (!fileLwc.is_open()) {
    cout << endl << "Error opening file " << lwcFileName << endl << endl;
    exit (1);
  }

  fileSca.open(scaFileName,ios::in | ios::binary);
  if (!fileSca.is_open()) {
    cout << endl << "Error opening file " << scaFileName << endl << endl;
    exit (1);
  }

  fileQtt.open(qttFileName,ios::in | ios::binary);
  if (!fileQtt.is_open()) {
    cout << endl << "Error opening file " << qttFileName << endl << endl;
    exit (1);
  }
  ...
  streamoff snow8Position;  // SeNorge snowsim
  streamoff snow16Position; // SeNorge snowsim
  k=0;
  //  for (i=0; i<nRows; i++) {
  //    for (j=0; j<nCols; j++) {
  for (i = 0; i<numberIndexStore; i++) {
	...
	if (DistHbv[k].GetGeoIndex() == indexStore[i]) {
	  ...
	  /* SeNorge snowsim */
	  //	  snow8Position = indexStore[i]*(sizeof(unsigned char));
	  //	  snow16Position = indexStore[i]*(sizeof(unsigned short int));
	  snow8Position = i*(sizeof(unsigned char));
	  snow16Position = i*(sizeof(unsigned short int));
	  ...
	  /* SeNorge snowsim */
	  fileSwe.seekg(snow16Position, ios::beg);
	  fileLwc.seekg(snow8Position, ios::beg);
	  fileSca.seekg(snow8Position, ios::beg);
	  fileQtt.seekg(snow8Position, ios::beg);
	  ...
	  /*          filePrec.read(reinterpret_cast<char *> (&(precip10[ELEMENT(i,j)])), sizeof (unsigned short int));
		  ...
		  /* SeNorge snowsim */
		  fileSwe.read(reinterpret_cast<char *> (&(swe10[indexStore[i]])), sizeof(unsigned short int));
		  fileLwc.read(reinterpret_cast<char *> (&(lwc10[indexStore[i]])), sizeof(unsigned char));
		  fileSca.read(reinterpret_cast<char *> (&(sca10[indexStore[i]])), sizeof(unsigned char));
		  fileQtt.read(reinterpret_cast<char *> (&(qtt10[indexStore[i]])), sizeof(unsigned char));
		  ...
		  if (precip10[indexStore[i]]<metMissing && temp10K[indexStore[i]]<metMissing) {
			  *inputDataFound = true;
			  ...
			  /* SeNorge snowsim */
			  if (swe10[indexStore[i]] == noData16u) {
			    sweSnowSim = 0.0;
			  } else {
			    sweSnowSim = (double)swe10[indexStore[i]] / 10000.0;
			  }
			  if (lwc10[indexStore[i]] == noSnow8u || lwc10[indexStore[i]] == noData8u) {
			    lwcSnowSim = 0.0;
			  } else {
			    lwcSnowSim = (double)lwc10[indexStore[i]] / 1000.0;
			  }
			  if (sca10[indexStore[i]] == noData8u) {
			    scaSnowSim = 0.0;
			  } else {
			    scaSnowSim = (double)sca10[indexStore[i]] / 100.0;
			  }
			  if (qtt10[indexStore[i]] == noData8u) {
			    qttSnowSim = precipitation;
			  } else {
			    qttSnowSim = (double)qtt10[indexStore[i]] / 1000.0;
			  }
			  ...
			  /* SeNorge snowsim */
			  InputElementStore->SetInput(7, sweSnowSim);
			  InputElementStore->SetInput(8, lwcSnowSim);
			  InputElementStore->SetInput(9, scaSnowSim);
			  InputElementStore->SetInput(10, qttSnowSim);
		  ...
		  /*else {
		    ...
		    *inputDataFound=true;
		    ...
		    /* SeNorge snowsim */
		    if (InputElementStore->GetInput(7) == missingData) InputElementStore->SetInput(7, 0.0);
		    if (InputElementStore->GetInput(8) == missingData) InputElementStore->SetInput(8, 0.0);
		    if (InputElementStore->GetInput(9) == missingData) InputElementStore->SetInput(9, 0.0);
		    if (InputElementStore->GetInput(10) == missingData) InputElementStore->SetInput(10, 5.0);
            // }
		  }
          // Snow store is removed at day no. DAY_SNOW_ZERO
          /*if (ParGeneralStore->GetDAY_SNOW_ZERO() > 0 &&
		  ...
		  /*cout << InputElementStore->GetInput(0) << " " 
		       << InputElementStore->GetInput(1) << " " 
		       << InputElementStore->GetInput(2) << " " 
		       << InputElementStore->GetInput(3) << " " 
		       << InputElementStore->GetInput(4) << " " 
		       << InputElementStore->GetInput(5) << " " 
		       << InputElementStore->GetInput(6) << " " */
		       /*<< InputElementStore->GetInput(7) << " " 
		       << InputElementStore->GetInput(8) << " " 
		       << InputElementStore->GetInput(9) << " " 
		       << InputElementStore->GetInput(10) << " "
		       << "\n"; */
                     ...
                     int nCols, int noData, double xllCorner, double yllCorner, double cellSize, ofstream &fout, int writePET)
{
  ...
  char dirName[100];
  char timeName[100];
  unsigned short int noData16u=65535;
  ...
  for (i=0; i<nRows; i++) {
    for (j=0; j<nCols; j++) {
      if (k<numLand) {
	    ...
	    swe[index] = (unsigned short int) ((DistHbv[k].GetSnowStore() + DistHbv[k].GetMeltWater()) * 1000); //mm (ingjerd: kan ikke ha *10, da kan du gå over unsigned short int grensa på 65535
          }
          else {
	    swe[index] = (unsigned short int) (noData16u);
          }
          k++;
        }
        else {
  ...
  cout << " k " << k << " numLand " << numLand << " index " << index << " nRows*nCols " << nRows*nCols << endl;
