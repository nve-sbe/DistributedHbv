...
double Snow::GetSnowCoverFraction()
{ 
/* SeNorge snowsim */
  snowCoverFraction = GetInputElement()->GetInput(9);
  return snowCoverFraction; 
}

double Snow::GetSnowStore()
{
/* SeNorge snowsim */
  return snowStore;
}

double Snow::GetMeltWater()
{
/* SeNorge snowsim */
  return meltWater;
}

double Snow::GetSnowWaterEquivalentChange()
{
  snowWaterEquivalentChange = GetInputElement()->GetInput(7) - initialSwe;
  initialSwe = GetInputElement()->GetInput(7);
  return snowWaterEquivalentChange;
}

double Snow::GetWaterOutput()
{
/* SeNorge snowsim */
  waterOutput = GetInputElement()->GetInput(10);
  return waterOutput;
}
...
void Snow::SetSnowsimValues()
{
  snowStore = GetInputElement()->GetInput(7) * (1.0 - GetInputElement()->GetInput(8));
  meltWater = GetInputElement()->GetInput(7) * GetInputElement()->GetInput(8);
}
...
void Lake::WaterBalance(int timeStep, DateTime datetime, int dayofyear_PM2)
{
  GetLakeWaterBalance()->WaterBalance(timeStep, datetime, dayofyear_PM2);
}
...
void Glacier::WaterBalance(int timeStep, DateTime datetime, int initialTimeSteps, int numberTimeSteps)
{
  ...
  GetHBV()->WaterBalance(timeStep, datetime, GetGlacierSurface()->GetIceMelt()*(1.0-GetSnow()->GetSnowCoverFraction()) 
                         + GetSnow()->GetWaterOutput(), GetSnow()->GetSnowCoverFraction(), missingData, missingData);
}
...
void HbvAquifer::WaterBalance(int timeStep, DateTime datetime, int dayofyear_PM2)
{
  GetVegetation()->WaterBalance(timeStep, datetime, dayofyear_PM2, GetSnow()->GetSnowCoverFraction());
 /* Hbv snow */
  //  GetSnow()->WaterBalance(timeStep, datetime, GetVegetation()->GetThroughFall());
  GetSnow()->SetSnowsimValues();
  //  GetHBV()->WaterBalance(timeStep, datetime, GetSnow()->GetWaterOutput(), GetSnow()->GetSnowCoverFraction(), GetVegetation()->GetDryPeriod(), GetVegetation()->GetPET_PM());
  /* SeNorge snowsim */
  GetHBV()->WaterBalance(timeStep, datetime, GetSnow()->GetWaterOutput() - GetVegetation()->GetInterceptionLoss(), GetSnow()->GetSnowCoverFraction(), GetVegetation()->GetDryPeriod(), GetVegetation()->GetPET_PM());
}
...
double DistributedHbv::GetPET() const
{
	double pet = 0.0;
	double sumAreaFraction = 0.0;
	bool elementFound = false;
